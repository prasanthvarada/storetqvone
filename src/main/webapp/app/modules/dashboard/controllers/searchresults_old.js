...
function ($rootScope, $scope, $state, $location, dashboardService, Flash, $http, sharedProperties) {
 ...
 $http({
        method : "GET",
       url : 'http://192.168.99.1:8080/api/storeby/'+$scope.DEPT_NUM+'/'+$scope.GROUPS+'/'+$scope.PAGE+'/'+$scope.dcnum+'/'+$scope.strnum
		//url : 'http://192.168.99.1:8080/api/storeby/'+$scope.DEPT_NUM+'/'+$scope.GROUPS+'/'+$scope.PAGE+'/'+$scope.dcnum+'/'+$scope.strnum
    }).then(function mySuccess(response) {
		$scope.check = true;
        $scope.fetcheddata = response.data;
		console.log("success in displaying");
...
console.log("inside state change");
	...
	//	var res = $http.post('http://127.0.0.1:3000/updateOTS', {"otsadj" :mes.otsadj,"origots":neworigots,"origpct":mes.origpct,"id" :mes.id});
		var res = $http.put('http://localhost:8080/api/updateOTS'+'/'+mes.otsadj+'/'+neworigots+'/'+mes.origpct+'/'+mes.id);
		res.success(function(data, status, headers, config) {
